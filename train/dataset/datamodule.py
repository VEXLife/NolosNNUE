...
def process_game(moves, scores, result):
    """
    Process a game and return a list of samples with rotations and flips.
    """
    samples = []
    scores = list(map(float, scores.strip('[]').split(';')))
    ...
    for i, (row, col) in enumerate(moves):
        ...
        if i > 0: # Skip the first move
            if winner == 0: # or i < len(moves) - 1:
                label = 0
            else:
                label = 1 if piece == winner else -1
            action = row * BOARD_SIZE + col
            samples.append((board.unsqueeze(0), next_board.unsqueeze(0), torch.FloatTensor([scores[i]]), torch.FloatTensor([label]), torch.LongTensor([action])))
        board = next_board
    ...
    augmented_samples = []
    for board, next_board, score, label, action in samples:
        ...
        for k in range(4):
            ...
            augmented_samples.append((rotated_board, rotated_next_board, score, label, rotated_action))
            ...
            augmented_samples.append((flipped_board, flipped_next_board, score, label, flipped_action))
...
def load_dataset(csv_paths):
    ...
    for csv_path in csv_paths:
        with open(csv_path, newline='', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                if not row or row[0] == "game_code":
                    continue
                move_str, scores, result = row[0], row[1], row[2]
                moves = parse_moves(move_str)
                samples = process_game(moves, scores, result)
                data.extend(samples)
    return data
...
class GomokuDataset(Dataset):
    def __init__(self, data):
        ...
        self.scores = [item[2] for item in data]
        self.labels = [item[3] for item in data]
        self.actions = [item[4] for item in data]

    def __len__(self):
        return len(self.boards)

    def __getitem__(self, idx):
        return self.boards[idx], self.next_boards[idx], self.scores[idx], self.labels[idx], self.actions[idx]
